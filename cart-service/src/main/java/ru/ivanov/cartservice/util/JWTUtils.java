...
//        return extractExpirationFromToken(token).after(new Date());
//    }
//
//    public List<GrantedAuthority> extractRolesFromToken(String token) {
//        List<?> roles = extractAllClaimsFromToken(token).get("roles", List.class);
//        return roles.stream()
...
//        return UUID.fromString(extractAllClaimsFromToken(token).getSubject());
//    }
//
